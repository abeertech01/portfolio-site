    ...
    <div className="lg:fixed lg:flex lg:flex-col lg:justify-between block lg:w-[47.3rem] lg:h-full h-auto lg:pt-[7rem] pt-[2rem] lg:mb-0 mb-[9rem]">
      ...
      <div className="mb-[2rem]">
        ...
        <div className="text-grayish mb-[3.5vh]">
          <h1 className="text-white text-[1.6rem] font-medium mb-3">
            Let&apos;s Connect
          </h1>
          <div className="flex flex-col gap-[0.5rem]">
            {/* Github  */}
            <div className="text-[1.5rem] text-white">
              Email: <a href="#">abeer.technology@gmail.com</a>
            </div>
            <div className="text-[1.5rem] text-white">
              Github:{" "}
              <a
                ...
                target="_blank"
                className="border-b-[0.1rem] border-sky-500 text-sky-500"
              >
                {/* <svg
        ...
        {/* Blog section */}
        <div className="text-grayish text-[1.4rem] lg:mb-0">
          <h1 className="text-white text-[1.4rem] font-medium mb-4">
            <a
              href="https://abeer.hashnode.dev"
              target="_blank"
              className="text-[1.5rem] font-normal border-b-[0.2rem] border-[#ffffff2a]"
            >
          ...
          <ul className="lg:w-5/6 relative -left-2 lg:-left-4">
            {blogPosts.map((post, index) => (
              <li
                key={index}
                className="mb-2 bg-[#5fffd409] rounded-xl border-t-[0.01rem] border-[#ffffff2a]"
              >
                <a href={post.link} target="_blank" className="w-full h-full">
                  <div className="flex gap-4 p-4">
                    <img
                      src={post.image}
                      alt={post.title}
                      className="w-16 h-16 object-cover rounded-md border border-spacing-2 border-[#eeeeee40]"
                    />
                    <div className="flex flex-col justify-between">
                      <h3 className="font-medium line-clamp-1 text-white text-[1.5rem]">
                        {post.title}
                      </h3>
                      <div className="line-clamp-1 text-[1.3rem]">
                        {post.preview}
                      </div>
                    </div>
                  </div>
